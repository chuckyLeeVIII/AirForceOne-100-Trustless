...
contract ContractEncryption {
    ...
    constructor(address _tokenAddress, uint256 _requiredBalance, bytes32 _encryptionKey) {
        tokenAddress = _tokenAddress;
        requiredBalance = _requiredBalance;
    ...
    function _encrypt(bytes memory data) internal view returns (bytes memory) {
        bytes memory result = new bytes(data.length);
        for(uint i = 0; i < data.length; i++) {
            result[i] = data[i] ^ bytes1(uint8(uint256(encryptionKey) >> (i % 32 * 8)));
        }
        return result;
    }

    function _decrypt(bytes memory encrypted) internal view returns (bytes memory) {
        return _encrypt(encrypted); // XOR encryption is reversible
    }
}
